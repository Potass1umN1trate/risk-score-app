...
_BLOCKCYPHER_URL = "https://api.blockcypher.com/v1/ltc/main/addrs"
_BLOCKCHAIR_URL = "https://api.blockchair.com/litecoin/dashboards/address"
_TIMEOUT = httpx.Timeout(15.0)


class LitecoinFetcher(BlockchainFetcher):
    """
    Fetches Litecoin transactions via BlockCypher public API.
    UTXO model — same normalization logic as Bitcoin.
    ...
    async def _fetch_raw(
        ...
        try:
            ...
            if resp.status_code == 429:
                raise BlockchainRateLimitedError(f"BlockCypher LTC rate-limited (HTTP 429) for {address}")
            ...
            logger.warning("BlockCypher LTC failed for %s: %s — trying Blockchair", address, exc)
        ...
        try:
            ...
            logger.error("Blockchair LTC also failed for %s: %s", address, exc)
            raise BlockchainUnavailableError(f"All LTC providers failed for {address}") from exc
        ...
        for tx in raw_txs:
            ...
            for out in outputs:
                ...
                if not out_addrs:
                    continue
                to_addr = out_addrs[0]
                amount_litoshi = int(out.get("value") or 0)
                ...
                result.append(Transaction(
                    ...
                    to_address=to_addr,
                    amount=float(amount_litoshi) / 1e8,
                    timestamp=timestamp,
                ))
