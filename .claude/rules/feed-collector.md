...
- Standalone Python skeleton (`feed_collector/`) with dry-run pipeline — no DB writes in dry-run mode; default source remains local dummy and makes no external API calls:
  ...
  - `feed_collector/README.md` — local dummy dry-run instructions plus Chainabuse and ScamSniffer source notes. Chainabuse has a manual `/v0/reports` dry-run smoke procedure using either `feed_collector/.env` or shell-only environment variables.
  ...
  - `feed_collector/app/mappings.py` — explicit source-aware mappings from feed-native chain/category values to project codes. Implemented for `dummy`, `chainabuse`, and `scamsniffer`; Chainabuse and ScamSniffer categories must flow through this mapping layer.
  ...
  - **`dry_run=True` does not connect to DB.** For Chainabuse, dry-run can make real `GET /v0/reports` calls during a manual smoke run, but it does not write to DB. `dry_run=False` requires `DATABASE_URL` (enforced by config validator) and writes to `flagged_addresses`, `flagged_address_sources`, `feed_sources` sync columns, and `audit_logs`.
  ...
  - Verified (2026-05-02): dry-run runs cleanly from repo root and from `feed_collector/` (`fetched=3 normalized=2 skipped=1 dry_run=True`); config raises `ValidationError` when `dry_run=False` and `database_url=None`; 86 tests pass; 19 integration tests are present and skipped unless `TEST_DATABASE_URL` is set.
...
- [ ] Incremental fetch logic (`fetch_since` path based on `last_success_at`; iteration 4 runtime still always calls `fetch_initial`)
- [ ] Full multi-source orchestration in runtime (`ENABLED_SOURCES` may be comma-separated, but only the first source is executed)
- [ ] Real source integrations other than Chainabuse and ScamSniffer address blacklist: OFAC/SDN, TRM Sanctions, BitcoinAbuse, CryptoScamDB
...
2.  For each source:
    ...
    c. Fetch initial records (`fetch_initial`). Iteration 4 runtime always uses
...
- ❌ Initial vs incremental source sync is not implemented yet; iteration 4 runtime still always calls `fetch_initial`
