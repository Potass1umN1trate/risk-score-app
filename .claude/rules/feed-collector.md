...
- Standalone Python skeleton (`feed_collector/`) with dry-run pipeline — no DB writes in dry-run mode; default source remains local dummy and makes no external API calls:
  ...
  - `feed_collector/app/models.py` — `FeedSourceConfig`, source-native `RawFeedRecord`, internal `NormalizedFlaggedAddress` with preserved evidence metadata (`source_chain`, `source_category`, `trusted`, `checked`, `first_seen`, `last_seen`, `raw_payload`), `FeedRunResult` dataclasses. `FeedRunResult` records `fetch_mode` (`initial`, `incremental`, or `repeat_full`) and the `fetch_since` timestamp used for incremental runs, plus resilience counters: `persisted_count`, `evidence_inserted_count`, `duplicate_count`, `record_error_count`, and `source_error_count`.
  - `feed_collector/app/error_sanitizer.py` — shared `sanitize_error()` helper for bounded single-line error messages. It redacts obvious Postgres URLs, Authorization headers, API key/token/password/secret assignments, omits huge XML/JSON-looking bodies, and caps stored/returned error text.
  ...
  - `feed_collector/app/pipeline.py` — `run_pipeline(source, settings, db_pool=None) -> FeedRunResult`; `dry_run=True` path: checks availability, fetches initial records, normalizes source-native records through `normalize_feed_record`, returns summary with `fetch_mode="initial"`, never connects to DB, and catches fetch exceptions as source-level failures; `dry_run=False` path: requires `db_pool`, looks up `feed_sources` by `source_code`, marks attempt, chooses fetch mode from `feed_sources.last_success_at` and `source.supports_time_filter`, normalizes, resolves mapped network/category ids, upserts `flagged_addresses` and inserts `flagged_address_sources`, marks success or failure, writes `FEED_COLLECT` audit log; if source code is not present/active in `feed_sources`, returns an error result without fetching. DB fetch modes are `initial` when `last_success_at IS NULL`, `incremental` when `last_success_at` is set and the source supports time filtering, and `repeat_full` when `last_success_at` is set but the source does not support time filtering. Failed incremental fetches do not fallback to initial fetch. Per-record DB processing is isolated: missing DB network/category mappings are deterministic skips, unexpected per-record DB exceptions increment `record_error_count` and processing continues, and duplicate evidence increments `duplicate_count` instead of erroring. If fetch succeeds and processing completes, `mark_feed_success` is called even when `skipped_count` or `record_error_count` is non-zero.
  - `feed_collector/app/repository.py` — asyncpg-based repository: `get_feed_source_by_code`, `mark_feed_attempt`, `mark_feed_success`, `mark_feed_failure`, `resolve_network_id`, `resolve_risk_category_id`, `upsert_flagged_address` (INSERT … ON CONFLICT DO NOTHING + SELECT fallback), `insert_flagged_address_source` (ON CONFLICT DO NOTHING, branches on `external_id`, stores preserved evidence fields), `write_audit_log` (includes fetch mode, incremental cursor timestamp, resilience counters, and bounded sanitized `error_samples` in `details_json`).
  ...
  - Unit tests in `feed_collector/tests/` cover models, source mappings, normalizer behavior, DummySource, ChainabuseSource with mocked httpx only, ScamSnifferSource with mocked httpx only, OfacSource with mocked httpx/XML fixtures only, config env-file path behavior, sanitizer redaction/truncation, dry-run pipeline, and per-record resilience with mocked repository calls; no test depends on live Chainabuse, live GitHub, live OFAC, or live API keys.
...
2.  For each source:
    ...
    d. For each fetched address record:
       ...
       - UNKNOWN/UNSUPPORTED NETWORK → log bounded sanitized sample and skip the record
       - Map source-native `source_category` to an internal risk_categories.code
       - UNKNOWN/UNSUPPORTED CATEGORY → log bounded sanitized sample and skip the record
       ...
       - Evidence conflict/duplicate → increment duplicate_count; not an error
       - Unexpected per-record DB exception → increment record_error_count, store bounded sanitized sample, continue to next record
    e. If fetch succeeded and record processing completed, set feed_sources.last_success_at = NOW(), last_error = NULL even when skipped_count or record_error_count is non-zero
...
- `last_success_at` — set at end of run when fetch succeeds and record processing completes, even if some records are skipped or hit isolated per-record errors; used as the next incremental cursor for sources with `supports_time_filter=True`
- `last_error` — set on source-level failure using sanitized bounded text; cleared on success
...
- Evidence record: `(feed_source_id, external_id, flagged_address_id)` when `external_id` is set; `(feed_source_id, flagged_address_id, source_category)` otherwise. Duplicate evidence conflicts are counted in `duplicate_count` and are not errors.

### Error handling contract:
- Source-level failures: source unavailable, fetch method raises, malformed whole-source response raised by adapter, DB pool unavailable before processing, or missing/inactive `feed_sources` row. These set `source_error_count=1`, do not update `last_success_at`, and call `mark_feed_failure` with sanitized text when `feed_source_id` is known.
- Record-level skips/errors: unsupported network/category, bad/missing address, missing DB network/category mappings, and one-record DB persistence exceptions. Deterministic skips increment `skipped_count`; unexpected one-record exceptions increment `record_error_count`; processing continues with remaining records.
- Error samples in `FeedRunResult.errors` and `audit_logs.details_json.error_samples` are bounded and sanitized. They must not include database URLs, Authorization headers, API keys/tokens/passwords/secrets, huge XML bodies, or huge JSON bodies.
...
- ✅ Per-record DB errors are isolated; valid remaining records continue processing
- ✅ Duplicate evidence is counted as duplicate, not error
- ✅ Completed fetch with record-level skips/errors still marks feed success and updates `last_success_at`
- ✅ Source-level failures do not update `last_success_at`
- ✅ Error samples are bounded and sanitized before result/audit/failure storage
